package ai.unicode.worldgdp.controller.api;
...
public class CityAPIController {
    ...
    public ResponseEntity<?> getCities(@PathVariable String countryCode, @RequestParam(name = "pageNo", defaultValue = "1") Integer pageNo) {
        try {
            return new ResponseEntity<java.util.List<City>>(cityDao.getCities(countryCode, pageNo), HttpStatus.OK);
        } catch (Exception ex) {
            System.out.println("Error while getting cities for country: {}" + countryCode + ex);
            return new ResponseEntity<String>("Error while getting cities", HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    ...
    public ResponseEntity<?> addCity(@PathVariable String countryCode, @Valid @RequestBody City city) {
        try {
            ...
            return new ResponseEntity<String>("Error while adding city", HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
